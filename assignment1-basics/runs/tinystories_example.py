...
def tinystories_example():

    tokenfile = "TinyStoriesV2-GPT4"
    bs, cl = 128, 128
    hyper_params = ConfigParams(batch_size=bs, context_length=cl,
                       vocab_size=10000,
                       d_model=512, d_ff=1344, num_layers=4,
                       num_heads=16, theta=10000, resume=False)

    total_tokens_proc = 327_680_000
    # (your batch size × total step count × context length
    max_steps = total_tokens_proc // (bs * cl)
    print("Maximum steps should be ", max_steps)
    hyper_params.checkpoint.interval = 5000
    ...
    file_encode_bin_from_vocab_merges(tokenfile, "exper")

    token_train_path = OUT_PATH / f"{tokenfile}-exper.bin"
